...
// Velocidad maxima plausible para un humano viajando (avion comercial ~900km/h,
// se deja margen). Por encima de esto, es fisicamente imposible que la misma
// persona haya hecho ambas transacciones.
...
function evaluateGeoImpossible(currentTx, history) {
  ...
  const hoursElapsed =
    (new Date(currentTx.receivedAt).getTime() - new Date(previousTx.receivedAt).getTime()) / 3_600_000;

  // Transacciones casi simultaneas en el mismo lugar: no hay division por
  // tiempo-cero que evaluar razonablemente, se trata como no-trigger.
  if (hoursElapsed <= 0) {
    return {
      triggered: false,
      ruleId: RULE_ID,
